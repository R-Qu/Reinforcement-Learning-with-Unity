...
public class ShoppingMalcom : MonoBehaviour
{

    public string character = "malcom";
    public GameObject salesman;
    SellingSalesman salesman_script;
    public NavMeshAgent agent_malcom;
    public Animator animator_malcom;
    ...
    void Start()
    {
        ...
        agent_malcom = this.gameObject.GetComponent<NavMeshAgent>();
        // Get the game object animator
        animator_malcom = this.gameObject.GetComponent<Animator>();
    ...
    void FixedUpdate()
    {
        ...
        Debug.Log("shoppingQuest Malcom:" + shoppingQuest);
        ...
        if ((shoppingQuest == 0) && (InputX <= 0.0f) && (salesman_script.shoppingStarted < 2))
        {
            ...
            if (salesman_script.shopOccuppied < 2)
            {
                ...
                question_malcom = Random.Range(1, 10);
                // Make the agent moves to the sales man
                agent_malcom.SetDestination(new Vector3(0.07f, 0.0f, 1.20f));
            }
        ...
        if (((salesman_script.shopOccuppied == 0) || (!shopFree)) && ((Vector3.Distance(transform.position, new Vector3(0.07f, 0.0f, 1.0f)) <= 0.5f) && !salesDone))
        {
            ...
            agent_malcom.isStopped = true;
            // Set the animator selection input.
            animator_malcom.SetFloat("InputX", InputX);
    ...
    void SalesDone()
    {
        ...
        agent_malcom.SetDestination(new Vector3(-6.5f, 0.0f, 33f));
