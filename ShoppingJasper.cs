...
public class ShoppingJasper : MonoBehaviour
{

    public string character = "jasper";
    public GameObject salesman;
    SellingSalesman salesman_script;
    public NavMeshAgent agent_jasper;
    public Animator animator_jasper;
    ...
    void Start()
    {
        ...
        agent_jasper = this.gameObject.GetComponent<NavMeshAgent>();
        // Get the game object animator
        animator_jasper = this.gameObject.GetComponent<Animator>();
    ...
    void FixedUpdate()
    {
        ...
        Debug.Log("shoppingQuest Jasper:" + shoppingQuest);
        ...
        if ((shoppingQuest == 0) && (InputX <= 0.0f) && (salesman_script.shoppingStarted < 2))
        {
            ...
            if (salesman_script.shopOccuppied < 2)
            {
                ...
                question_jasper = Random.Range(1, 10);
                // Make the agent moves to the sales man
                agent_jasper.SetDestination(new Vector3(0.07f, 0.0f, 1.20f));
            }
            // Change the animation to Talking
            InputX = 1.0f;
        }
        Debug.Log("shopFree Jasper:" + shopFree);
        Debug.Log("salesDone Jasper:" + salesDone);
        ...
        if (((salesman_script.shopOccuppied == 0) || (!shopFree)) && ((Vector3.Distance(transform.position, new Vector3(0.07f, 0.0f, 1.0f)) <= 0.5f) && !salesDone))
        {
            ...
            agent_jasper.isStopped = true;
            // Set the animator selection input.
            animator_jasper.SetFloat("InputX", InputX);
    ...
    void SalesDone()
    {
        ...
        agent_jasper.SetDestination(new Vector3(-6.5f, 0.0f, 33f));
